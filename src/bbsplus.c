...
// TODO: could refactor with regular bbsplus_sign to be prevent duplication
void
bbsplus_sign_commitment(struct bbsplus_sign *sign, g1_t A,
        struct bbsplus_pk *pk, struct bbsplus_sk *sk) {
    bn_null(sign->e);
    bn_new(sign->e);
    bn_null(sign->s);
    bn_new(sign->s);
    ...
    g1_null(sign->A);
    g1_new(sign->A);
    g1_mul(sign->A, pk->bases[0], sign->s);
    g1_add(sign->A, sign->A, A); 
...
bbsplus_verify(struct bbsplus_sign *sign, struct bbsplus_pk *pk, bn_t *msgs, size_t nr_msgs) {
    ...
    int result = gt_cmp(gt1, gt2) == RLC_EQ;
...
bbsplus_prove(struct bbsplus_proof *proof,
        struct bbsplus_sign *sign, struct bbsplus_pk *pk,
        bn_t *msgs, size_t nr_msgs,
        unsigned int *hidden, size_t nr_hidden,
        uint8_t *L, size_t lL) {
    ...
    bbsplus_proof_randomizers(&rand, pk, nr_msgs, hidden, nr_hidden);
    bbsplus_proof_commitments(&coms, &rand, sign, pk, nr_msgs, hidden, nr_hidden);
...
bbsplus_proof_randomizers(struct bbsplus_proof_randomizers *rand,
    ...
    rand->mhat = malloc(nr_hidden * sizeof(bn_t));
...
bbsplus_proof_randomizers_free(struct bbsplus_proof_randomizers *rand) {
    ...
    for(int i = 0; i < rand->nr_hidden; i++) {
        bn_free(rand->mhat[i]);
    }
    free(rand->mhat);
}
...
bbsplus_proof_create(struct bbsplus_proof *proof,
    ...
    proof->hidden = malloc(nr_hidden * sizeof(unsigned int));
    for(int i = 0; i < nr_hidden; i++) {
        proof->hidden[i] = hidden[i];
    }
    proof->nr_hidden = nr_hidden;
    ...
    proof->mresps = malloc(nr_hidden * sizeof(bn_t));
    for(int i = 0; i < nr_hidden; i++) {
        bn_null(proof->mresps[i]);
        bn_new(proof->mresps[i]);
        bn_mul(proof->mresps[i], proof->challenge, msgs[hidden[i]]);
        bn_add(proof->mresps[i], proof->mresps[i], rand->mhat[i]);
        bn_mod(proof->mresps[i], proof->mresps[i], pk->q);
    }

    unsigned int *disclosed_idx;
    size_t nr_disclosed = nr_msgs - nr_hidden;
    complement_of_indices(&disclosed_idx, hidden, nr_hidden,
            nr_msgs);
    proof->disclosed = malloc(nr_disclosed * sizeof(bn_t));
    for(int i = 0; i < nr_disclosed; i++) {
        bn_null(proof->disclosed[i]);
        bn_new(proof->disclosed[i]);
        bn_copy(proof->disclosed[i], msgs[disclosed_idx[i]]);
    }
    free(disclosed_idx);
...
bbsplus_proof_free(struct bbsplus_proof *proof) {
    ...
    for(int i = 0; i < proof->nr_hidden; i++) {
        bn_free(proof->mresps[i]);
        bn_free(proof->disclosed[i]);
    }
    free(proof->mresps);
    free(proof->disclosed);
    free(proof->hidden);
...
bbsplus_proof_reconstruct_commitments( struct bbsplus_proof_commitments *coms,
    ...
    // Phat = (e(g1, g2)^{-1} * e(C1, pk->w) *
    //         prod_{i \in disclosed} e(base[i + 1], g2)^(-msg[i]))^{-challenge}
    //        (e(C1, g2)^{-1})^{eresp} * e(base[0], g2)^{sresp} *
    //        e(g1bar, pk->w)^{r1resp} * e(g1bar, g2)^{a1resp} *
    //        prod_{i \in hidden} e(base[i + 1], g2)^{mresp[i]}
    gt_null(coms->Phat);
    gt_new(coms->Phat);
    ...
    // pHat = e(g1, g2)*e(C1, pk->w)^{-1}
    pc_map(tmpt, proof->C1, pk->w);
    gt_inv(tmpt, tmpt);
    gt_mul(coms->Phat, pk->pairing_g1_g2, tmpt);

    // pHat = e(g1, g2)*e(C1, pk->w)^{-1} *
    //        prod_{i \in disclosed} e(base[i + 1], g2)^msg[i]
    unsigned int *disclosed_idx;
    size_t nr_disclosed = nr_msgs - proof->nr_hidden;
    complement_of_indices(&disclosed_idx, proof->hidden, proof->nr_hidden,
            nr_msgs);
    for(int i = 0; i < nr_disclosed; i++) {
        int idx = disclosed_idx[i];
        gt_exp(tmpt, pk->pairing_bases_g2[idx + 1], proof->disclosed[i]);
        gt_mul(coms->Phat, coms->Phat, tmpt);
    }
    free(disclosed_idx);
    ...
    for(int i = 0; i < proof->nr_hidden; i++) {
        ...
        gt_exp(tmpt, pk->pairing_bases_g2[proof->hidden[i] + 1], proof->mresps[i]);
        gt_mul(coms->Phat, coms->Phat, tmpt);
    }
    // printf("--- Phat:\n"); gt_print(coms->Phat);
}
...
bbsplus_proof_verify(struct bbsplus_proof *proof, struct bbsplus_pk *pk, size_t nr_msgs, uint8_t *L, size_t lL) {
    ...
    return bn_cmp(challenge, proof->challenge) == RLC_EQ;
}
